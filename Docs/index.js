    ...
    function compile(docarr) {
        ...
        docarr.forEach(docobj => {
            ...
            if (docobj.type == 'h1') {
                ...
                console.log(lineul);
                doccontent += `<br><div style="text-align: right;height: 30px;width: 100%;background-color: #333;color: white;width: 100%;border-radius: 5px 5px 0 0;margin: 0 5px 0 5px;"><p style="margin-right: 10px;">${docobj.lang}</p></div><div style="display: grid;grid-template-columns: auto 1fr;grid-template-rows: auto;grid-gap: 10px;background-color: #333;color: white;width: 100%;height: auto;border-radius: 0 0 5px 5px;margin: 0 5px 0 5px;"><ul style="list-style-type: none;padding-left: 10px;">${lineul}</ul><pre style="margin-top: 16px;"><code class="language-${docobj.lang}" style="line-height: 1.6;" title="${docobj.lang}">${hljs.highlight((docobj.code).replaceAll('%Enter%', '\n'), {language: docobj.lang}).value}</code></pre></div>`;
