    ...
    constructor(id, config) {
        ...
        config.forEach((item, index) => {
            if (item.type == "text") {
                ...
                renderid.innerHTML += `<div class="form form-input">${item.label}<input placeholder="${item.placeholder}" type="${item.valueType}" min="${item.min}" max="${item.max}" value="${item.value}" ${item.disabled ? 'disabled="true"' : ""}"></div>`
            }
        })
